...
app.put('/api/articulos/:sku', (req, res) => {
  const { sku } = req.params;
  const { nombre, marca, cantidad, nuevoSku } = req.body;

  // Verificar si el nuevo SKU ya existe y es diferente al actual
  if (nuevoSku && nuevoSku !== sku) {
    db.query('SELECT * FROM articulos WHERE sku = ?', [nuevoSku], (err, results) => {
      ...
      // Actualizar el SKU y demás datos
      const sql = `UPDATE articulos SET sku = ?, nombre = ?, marca = ?, cantidad = ? WHERE sku = ?`;
      db.query(sql, [nuevoSku, nombre, marca, cantidad, sku], (err, result) => {
        if (err) return res.status(500).send(err.message);
        res.status(200).send('Artículo actualizado (con cambio de SKU)');
      });
    });
  } else {
    // Actualizar los demás datos (sin cambiar el SKU)
    const sql = `UPDATE articulos SET nombre = ?, marca = ?, cantidad = ? WHERE sku = ?`;
    db.query(sql, [nombre, marca, cantidad, sku], (err, result) => {
      if (err) return res.status(500).send(err.message);
